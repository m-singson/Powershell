<#	
	...
	############################################################################ 
	 ...
	 Notes:
		...
		ExpireInDays: Amount of days before a password is set to expire.
      		EmailBody: Enter the email you want users to recieve, this can be a HTML email.
      		DirPath: Directory where you've stored the script.
...
#EmailBody File. The -Raw gets the content as a whole string rather than processing it line by line.
$Emailbody = Get-Content -Path "\\FilePath\EmailBody.html" -raw
...
foreach ($user in $users)
{
    ...
    $mailParams = @{
                  ...
                  From                       = 'from@email.com'
                  To                         = 'to@email.com'
                  Subject                    = "Your Password Is Due to Expire in $daystoexpire day(s)"
                  Priority                   = "High"
                  Body                       = $emailbody
